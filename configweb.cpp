...
void initConfigWebService() {
  ...
  //         AsyncWebServerResponse *response = request->beginResponse(
  //             200, "text/plain",
  //             shouldReboot ? "Data has uploaded successfully, will be
  //             rebooted!"
  //                          : "FAIL!");
  //         response->addHeader("Connection", "close");
  //         request->send(response);
  ...
  //     // for (uint8_t idx = 0; idx < sizeof(config.segments) /
  //     // sizeof(TimeSegment); idx++)
  //     // {
  ...
  //   server.onFileUpload([](AsyncWebServerRequest *request, const String
  //   &filename,
  ...
  //   server.addHandler(new SPIFFSEditor(SPIFFS, HTTP_EDIT_USER,
  //   HTTP_EDIT_PASS));
}
