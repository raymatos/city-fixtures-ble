...
class SystemTimeCallback : public BLECharacteristicCallbacks {
  void onWrite(BLECharacteristic *pCharacteristic) {
    ...
    if (value.length() > 0) {
      ...
      // Extract time
      String timeStamp =
          formattedDate.substring(splitT + 1, formattedDate.length() - 1);
...
void init_ble_service() {
  ...
  pForceRelayCharacteristic = pService->createCharacteristic(
      FORCERELAY_CHARACTERISTIC_UUID, BLECharacteristic::PROPERTY_WRITE);
...
void ble_stop_service() { BLEDevice::deinit(true); }
...
void ble_handler_loop() {
  ...
  if (deviceConnected) {
    ...
    if (millis() - notify_time >= 1000) {
      ...
      pPeripheralNotifyCharacteristic->setValue((uint8_t *)payload.c_str(),
                                                payload.length());
    ...
    if (millis() - datetime_notify_time >= 5000) {
      ...
      char payload[20];
      sprintf(payload, "%d-%02d-%02dT%02d:%02d", tmstruct.tm_year + 1900,
  ...
  if (!deviceConnected && oldDeviceConnected) {
    delay(500); // give the bluetooth stack the chance to
    // get things ready pServer->startAdvertising(); // restart advertising
