...
/* STATIC */ BLEClient *BLEDevice::createClient() {
  log_v(">> createClient");
#ifndef CONFIG_GATTC_ENABLE // Check that BLE GATTC is enabled in make
                            // menuconfig
...
/* STATIC */ BLEServer *BLEDevice::createServer() {
  log_v(">> createServer");
#ifndef CONFIG_GATTS_ENABLE // Check that BLE GATTS is enabled in make
                            // menuconfig
 ...
 */
/* STATIC */ void
BLEDevice::gattServerEventHandler(esp_gatts_cb_event_t event,
...
#ifdef CONFIG_BLE_SMP_ENABLE // Check that BLE SMP (security) is configured in
                             // make menuconfig
    if (BLEDevice::m_securityLevel) {
      esp_ble_set_encryption(param->connect.remote_bda,
                             BLEDevice::m_securityLevel);
    }
 ...
 */
/* STATIC */ void
BLEDevice::gattClientEventHandler(esp_gattc_cb_event_t event,
  ...
  for (auto &myPair : BLEDevice::getPeerDevices(true)) {
    ...
    if (((BLEClient *)conn_status.peer_device)->getGattcIf() == gattc_if ||
        ((BLEClient *)conn_status.peer_device)->getGattcIf() ==
            ESP_GATT_IF_NONE ||
        gattc_if == ESP_GATT_IF_NONE) {
      ((BLEClient *)conn_status.peer_device)
          ->gattClientEventHandler(event, gattc_if, param);
    }
  }
...
/* STATIC */ void BLEDevice::gapEventHandler(esp_gap_ble_cb_event_t event,
  ...
  case ESP_GAP_BLE_NC_REQ_EVT: /*  NUMERIC CONFIRMATION  */
    ...
    if (BLEDevice::m_securityCallbacks != nullptr) {
      esp_ble_confirm_reply(param->ble_security.ble_req.bd_addr,
                            BLEDevice::m_securityCallbacks->onConfirmPIN(
  ...
  case ESP_GAP_BLE_PASSKEY_NOTIF_EVT: // the app will receive this evt when the
    ...
    // display the passkey number to the user to input it in the peer deivce
    // within 30 seconds
    ...
    if (BLEDevice::m_securityCallbacks != nullptr) {
      BLEDevice::m_securityCallbacks->onPassKeyNotify(
          param->ble_security.key_notif.passkey);
    }
#endif // CONFIG_BLE_SMP_ENABLE
    break;
...
#ifdef CONFIG_BLE_SMP_ENABLE // Check that BLE SMP (security) is configured in
                             // make menuconfig
    log_i("key type = %s", BLESecurity::esp_key_type_to_str(
  ...
  case ESP_GAP_BLE_AUTH_CMPL_EVT:
    log_i("ESP_GAP_BLE_AUTH_CMPL_EVT");
                             ...
                             // make menuconfig
    if (BLEDevice::m_securityCallbacks != nullptr) {
      BLEDevice::m_securityCallbacks->onAuthenticationComplete(
 ...
 */
/* STATIC */ std::string BLEDevice::getValue(BLEAddress bdAddress,
                                             BLEUUID serviceUUID,
...
/* STATIC */ void BLEDevice::init(std::string deviceName) {
  if (!initialized) {
    initialized = true; // Set the initialization flag to ensure we are only
                        // initialized once.
    ...
    if (errRc != ESP_OK) {
      log_e("nvs_flash_init: rc=%d %s", errRc,
    ...
    if (errRc != ESP_OK) {
      log_e("esp_bt_controller_init: rc=%d %s", errRc,
            GeneralUtils::errorToString(errRc));
      return;
    }
    ...
    if (errRc != ESP_OK) {
      log_e("esp_bt_controller_enable: rc=%d %s", errRc,
            GeneralUtils::errorToString(errRc));
      return;
    }
#else
    ...
    if (bt_state == ESP_BLUEDROID_STATUS_UNINITIALIZED) {
      ...
      if (errRc != ESP_OK) {
        log_e("esp_bluedroid_init: rc=%d %s", errRc,
              GeneralUtils::errorToString(errRc));
        return;
      }
    }
    ...
    if (bt_state != ESP_BLUEDROID_STATUS_ENABLED) {
      ...
      if (errRc != ESP_OK) {
        log_e("esp_bluedroid_enable: rc=%d %s", errRc,
              GeneralUtils::errorToString(errRc));
        return;
      }
    }
    ...
    if (errRc != ESP_OK) {
      log_e("esp_ble_gap_register_callback: rc=%d %s", errRc,
            GeneralUtils::errorToString(errRc));
      return;
    }

#ifdef CONFIG_GATTC_ENABLE // Check that BLE client is configured in make
                           // menuconfig
    errRc = esp_ble_gattc_register_callback(BLEDevice::gattClientEventHandler);
    if (errRc != ESP_OK) {
      log_e("esp_ble_gattc_register_callback: rc=%d %s", errRc,
            GeneralUtils::errorToString(errRc));
      return;
    }
#endif // CONFIG_GATTC_ENABLE

#ifdef CONFIG_GATTS_ENABLE // Check that BLE server is configured in make
                           // menuconfig
    errRc = esp_ble_gatts_register_callback(BLEDevice::gattServerEventHandler);
    if (errRc != ESP_OK) {
      log_e("esp_ble_gatts_register_callback: rc=%d %s", errRc,
            GeneralUtils::errorToString(errRc));
      return;
    }
#endif // CONFIG_GATTS_ENABLE
    ...
    if (errRc != ESP_OK) {
      log_e("esp_ble_gap_set_device_name: rc=%d %s", errRc,
            GeneralUtils::errorToString(errRc));
      return;
    };
    ...
    errRc = ::esp_ble_gap_set_security_param(ESP_BLE_SM_IOCAP_MODE, &iocap,
                                             sizeof(uint8_t));
    if (errRc != ESP_OK) {
      log_e("esp_ble_gap_set_security_param: rc=%d %s", errRc,
            GeneralUtils::errorToString(errRc));
      return;
    };
#endif // CONFIG_BLE_SMP_ENABLE
  ...
  vTaskDelay(200 /
             portTICK_PERIOD_MS); // Delay for 200 msecs as a workaround to an
                                  // apparent Arduino environment issue.
} // init
...
/* STATIC */ void BLEDevice::setPower(esp_power_level_t powerLevel) {
  ...
  if (errRc != ESP_OK) {
    log_e("esp_ble_tx_power_set: rc=%d %s", errRc,
          GeneralUtils::errorToString(errRc));
  };
  log_v("<< setPower");
} // setPower
...
void BLEDevice::whiteListAdd(BLEAddress address) {
  ...
  esp_err_t errRc = esp_ble_gap_update_whitelist(
      true, *address.getNative()); // True to add an entry.
  if (errRc != ESP_OK) {
    log_e("esp_ble_gap_update_whitelist: rc=%d %s", errRc,
          GeneralUtils::errorToString(errRc));
  }
  log_v("<< whiteListAdd");
} // whiteListAdd
...
void BLEDevice::whiteListRemove(BLEAddress address) {
  ...
  esp_err_t errRc = esp_ble_gap_update_whitelist(
      false, *address.getNative()); // False to remove an entry.
  if (errRc != ESP_OK) {
    log_e("esp_ble_gap_update_whitelist: rc=%d %s", errRc,
          GeneralUtils::errorToString(errRc));
  }
...
/*
 * @brief Set encryption level that will be negotiated with peer device durng
 * connection
...
/*
 * @brief Set callbacks that will be used to handle encryption negotiation
 * events and authentication events
 ...
 * client peer
 * @param [in] mtu Value to set local mtu, should be larger than 23 and lower or
 * equal to 517
 */
...
void BLEDevice::updatePeerDevice(void *peer, bool _client, uint16_t conn_id) {
  log_d("update conn_id: %d, GATT role: %s", conn_id,
        _client ? "client" : "server");
  std::map<uint16_t, conn_status_t>::iterator it =
      m_connectedClientsMap.find(ESP_GATT_IF_NONE);
...
void BLEDevice::addPeerDevice(void *peer, bool _client, uint16_t conn_id) {
  log_i("add conn_id: %d, GATT role: %s", conn_id,
        _client ? "client" : "server");
  conn_status_t status = {.peer_device = peer, .connected = true, .mtu = 23};

  m_connectedClientsMap.insert(
      std::pair<uint16_t, conn_status_t>(conn_id, status));
}
...
/* STATIC */ void BLEDevice::deinit(bool release_memory) {
  ...
  if (release_memory) {
    esp_bt_controller_mem_release(
        ESP_BT_MODE_BTDM); // <-- require tests because we released classic BT
                           // memory and this can cause crash (most likely not,
                           // esp-idf takes care of it)
  } else {
    initialized = false;
  }
#endif
